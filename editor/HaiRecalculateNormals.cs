...
// Script belongs to Hai~
// https://github.com/hai-vr/
// https://docs.hai-vr.dev/docs/products/prefabulous/universal/recalculate-normals

namespace RykerTM.Tools.Hai
{
	public class NormalCalculator
	{
		public static void RecalculateNormalsOf(SkinnedMeshRenderer smr, List<string> thatSmrBlendShapes, List<string> applicableBlendShapes, List<string> eraseCustomSplitNormalsBlendShapes, bool doSave = true)
        {
            ...
            string list = "";
			foreach (string applicableBlendShape in applicableBlendShapes) // List applicable blendshapes to calculate
			{
				list = list + applicableBlendShape + "\n";
			}
            ...
            var bonesInBindpose = originalMesh.bindposes
                .Select(bindPose =>
                {
                    ...
                    ExtractFromTRS(inverse, out var pos, out var rot, out var sca);
                    t.localPosition = pos;
                    t.localRotation = rot;
                    t.localScale = sca;
            ...
            RebuildBlendshapesOnCopy(originalMesh, mesh, nameToFrameDeltaBakes);
            smr.sharedMesh = originalMesh;
			if (doSave) AssetDatabase.SaveAssets();
        }
        ...
        private static List<int[]> StoreIndicesWithSamePositionAndNormal(Mesh bake0Mesh)
        {
